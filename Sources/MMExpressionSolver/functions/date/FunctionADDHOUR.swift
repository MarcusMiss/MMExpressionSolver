...
/// Function ADDHOUR:()
///
/// Implementation of expression-function `ADDHOUR:()`.
///
/// Add given hour to given date.
///
/// ```
/// ADDHOUR:(value: date, year: int) -> date
/// ```
///
/// @Small { Available since <doc:MMExpressionSolver-Release-History#Release-1.2.0>. }
public final class FunctionADDHOUR: ExpressionFunction {

    /// Symbol of this function
    static let symbolFunction: String = "ADDHOUR"

    // MARK: - Protocol Identifiable

    public var id: FunctionIdentifier { get { FunctionIdentifier("FunctionADDHOUR") }}

    // MARK: - Protocol ExpressionFunction
    
    public var definition: ExpressionFunctionDefinition = ExpressionFunctionDefinition(
        name: FunctionADDHOUR.symbolFunction,
        parameters:[
    ...
    public func evaluateFunction(expression: MMExpression,
        ...
        if !p2.isIntegerValue {
            throw ExpressionError.invalidParameterType(token: functionToken,
                                                       funcName: definition.name,
                                                       paramName: ExpressionFunctionParameter.nameHour,
                                                       value: p2.asStringForError())
        }
        guard let result: Date = context.calendar.date(byAdding: DateComponents(hour: p2.asInteger()!), to: p1.asDateTime()!) else {
    ...
    public var description: String {
        return FunctionADDHOUR.symbolFunction
    }
