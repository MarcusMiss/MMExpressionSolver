...
@Suite("Function UUID()")
class FunctionUUIDTests {

    @Test("Definition of UUID()")
    func validateInit() async throws {
        let f: FunctionUUID = FunctionUUID()
        #expect(f.id == FunctionIdentifier("FunctionUUID"))
        #expect(f.symbols == ["UUID"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "UUID", parameters:[]))
    ...
    func validateSuccess() async throws {
        ...
        #expect(evalValue.asString()!.count == 36)
    }
