...
@Suite("Function RTRIM()")
class FunctionRTRIMTests {

    @Test("Definition of RTRIM()")
    func validateInit() async throws {
        let f: FunctionRTRIM = FunctionRTRIM()
        #expect(f.id == FunctionIdentifier("FunctionRTRIM"))
        #expect(f.symbols == ["RTRIM"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "RTRIM", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "RTRIM('   Lorem   ')"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
