...
class FunctionREPLACEALLTests {

    @Test("Validation of Utility")
    func validateUtility() async throws {
        #expect((FunctionREPLACEALL.replaceAll("Hello World", "o", "X") == "HellX WXrld") == true)
        #expect((FunctionREPLACEALL.replaceAll("Hello World", "x", "X") == "Hello World") == true)
    }

    @Test("Definition of REPLACEALL()")
    func validateInit() async throws {
        let f: FunctionREPLACEALL = FunctionREPLACEALL()
        #expect(f.id == FunctionIdentifier("FunctionREPLACEALL"))
        #expect(f.symbols == ["REPLACEALL"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "REPLACEALL", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "REPLACEALL('Lorem Ipsum', 'm', 'X')"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asString()! == "LoreX IpsuX")
    }
