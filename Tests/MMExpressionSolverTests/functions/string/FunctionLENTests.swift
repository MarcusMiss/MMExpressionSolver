...
@Suite("Function LEN()")
class FunctionLENTests {

    @Test("Definition of LEN()")
    func validateInit() async throws {
        let f: FunctionLEN = FunctionLEN()
        #expect(f.id == FunctionIdentifier("FunctionLEN"))
        #expect(f.symbols == ["LEN"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "LEN", parameters:[
    ...
    func evaluateFunction() async throws {
        ...
        try #require(result.asInteger() == 0)
        // 02
        result = try f.evaluateFunction(expression: fake.get(),
                                        context: ctxt,
                                        functionToken: mockToken,
        ...
        try #require(result.isIntegerValue == true)
        try #require(result.asInteger() == 5)
    }
    ...
    func validateSuccess() async throws {
        let expression: String = "LEN('Lorem Ipsum')"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asInteger()! == Int(11))
    }
