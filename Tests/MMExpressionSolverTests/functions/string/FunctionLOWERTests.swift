...
@Suite("Function LOWER()")
class FunctionLOWERTests {

    @Test("Definition of LOWER()")
    func validateInit() async throws {
        let f: FunctionLOWER = FunctionLOWER()
        #expect(f.id == FunctionIdentifier("FunctionLOWER"))
        #expect(f.symbols == ["LOWER"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "LOWER", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "LOWER('Lorem Ipsum')"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asString()! == "lorem ipsum")
    }
