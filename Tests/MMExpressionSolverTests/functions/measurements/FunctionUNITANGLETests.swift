...
@Suite("Function UNITANGLE()")
class FunctionUNITANGLETests {

    @Test("solveUnitSymbol of UNITANGLE()")
    func solveUnitSymbol() async throws {
        try #require(FunctionUNITANGLE.solveUnitSymbol("°")! == .degrees)
        try #require(FunctionUNITANGLE.solveUnitSymbol("ʹ")! == .arcMinutes)
        try #require(FunctionUNITANGLE.solveUnitSymbol("ʺ")! == .arcSeconds)
        try #require(FunctionUNITANGLE.solveUnitSymbol("rad")! == .radians)
        try #require(FunctionUNITANGLE.solveUnitSymbol("grad")! == .gradians)
        try #require(FunctionUNITANGLE.solveUnitSymbol("rev")! == .revolutions)
    }

    @Test("Definition of UNITANGLE()")
    func validateInit() async throws {
        let f: FunctionUNITANGLE = FunctionUNITANGLE()
        #expect(f.id == FunctionIdentifier("FunctionUNITANGLE"))
        #expect(f.symbols == ["UNITANGLE"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "UNITANGLE", parameters:[
    ...
    @Test("Function evaluation in expression-context",
        arguments: [
            "UNITANGLE(NULL, 'murks')",
            "UNITANGLE(20.0, \"murks\")",
            "UNITANGLE(var1, var1)",
        ]
    )
    ...
    func validateSuccess() async throws {
        let expression: String = "UNITANGLE(100.0, \"°\")"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        let context: ExpressionEvaluationContextImpl = ExpressionEvaluationContextImpl(configuration: config)
        let result: Result<MMExpression, ExpressionError> = MMExpression.build(
        ...
        #expect(evalValue.isUnitAngle)
        #expect(evalValue.asUnitAngle()! == Measurement<UnitAngle>(value: 100.0, unit: .degrees))
    }
