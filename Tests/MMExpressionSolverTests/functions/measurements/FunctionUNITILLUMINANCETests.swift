//
//  FunctionUNITILLUMINANCETests.swift
//  MMExpressionSolver
//
...
@Suite("Function UNITILLUMINANCE()")
class FunctionUNITILLUMINANCETests {

    @Test("solveUnitSymbol of UNITILLUMINANCE()")
    func solveUnitSymbol() async throws {
        try #require(FunctionUNITILLUMINANCE.solveUnitSymbol("lx")! == .lux)
    }

    @Test("Definition of UNITILLUMINANCE()")
    func validateInit() async throws {
        let f: FunctionUNITILLUMINANCE = FunctionUNITILLUMINANCE()
        #expect(f.id == FunctionIdentifier("FunctionUNITILLUMINANCE"))
        #expect(f.symbols == ["UNITILLUMINANCE"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "UNITILLUMINANCE", parameters:[
    ...
    @Test("Function evaluation in expression-context",
        arguments: [
          "UNITILLUMINANCE(10, 'lx')",
          "UNITILLUMINANCE(var1, 'lx')",
        ]
    )
    ...
    @Test("Function evaluation in expression-context",
        arguments: [
            "UNITILLUMINANCE(NULL, 'murks')",
            "UNITILLUMINANCE(20.0, \"murks\")",
            "UNITILLUMINANCE(var1, var1)",
        ]
    )
    ...
    func validateSuccess() async throws {
        let expression: String = "UNITILLUMINANCE(100.0, \"lx\")"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        let context: ExpressionEvaluationContextImpl = ExpressionEvaluationContextImpl(configuration: config)
        let result: Result<MMExpression, ExpressionError> = MMExpression.build(
        ...
        #expect(evalValue.isUnitIlluminance)
        #expect(evalValue.asUnitIlluminance()! == Measurement<UnitIlluminance>(value: 100.0, unit: .lux))
    }
