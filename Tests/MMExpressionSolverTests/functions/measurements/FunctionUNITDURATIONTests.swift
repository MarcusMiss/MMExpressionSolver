//
//  FunctionUNITDURATIONTests.swift
//  MMExpressionSolver
//
...
@Suite("Function UNITDURATION()")
class FunctionUNITDURATIONTests {

    @Test("solveUnitSymbol of UNITDURATION()")
    func solveUnitSymbol() async throws {
        try #require(FunctionUNITDURATION.solveUnitSymbol("sec")! == .seconds)
        try #require(FunctionUNITDURATION.solveUnitSymbol("min")! == .minutes)
        try #require(FunctionUNITDURATION.solveUnitSymbol("hr")! == .hours)
    }

    @Test("Definition of UNITDURATION()")
    func validateInit() async throws {
        let f: FunctionUNITDURATION = FunctionUNITDURATION()
        #expect(f.id == FunctionIdentifier("FunctionUNITDURATION"))
        #expect(f.symbols == ["UNITDURATION"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "UNITDURATION", parameters:[
    ...
    @Test("Function evaluation in expression-context",
        arguments: [
            "UNITDURATION(NULL, 'murks')",
            "UNITDURATION(20.0, \"murks\")",
            "UNITDURATION(var1, var1)",
        ]
    )
    ...
    func validateSuccess() async throws {
        let expression: String = "UNITDURATION(100.0, \"sec\")"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        let context: ExpressionEvaluationContextImpl = ExpressionEvaluationContextImpl(configuration: config)
        let result: Result<MMExpression, ExpressionError> = MMExpression.build(
        ...
        #expect(evalValue.isUnitDuration)
        #expect(evalValue.asUnitDuration()! == Measurement<UnitDuration>(value: 100.0, unit: .seconds))
    }
