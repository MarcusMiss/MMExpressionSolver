...
@Suite("Function CBRT()")
class FunctionCBRTTests {

    @Test("Definition of SQRT()")
    func validateInit() async throws {
        let f: FunctionCBRT = FunctionCBRT()
        #expect(f.id == FunctionIdentifier("FunctionCBRT"))
        #expect(f.symbols == ["CBRT"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "CBRT", parameters:[
    ...
    func evaluateInExpression(expressionSource: String) async throws {
        ...
        context.variables.storeVariable(identifier: "var2", value: ExpressionValue.of((Double(64.0))))
        context.variables.storeVariable(identifier: "var3", value: ExpressionValue.of((Float(64.0))))
        context.variables.storeVariable(identifier: "var4", value: ExpressionValue.of((Decimal(64.0))))
    ...
    func evaluateInExpressionWithFailure(expressionSource: String) async throws {
        ...
        context.variables.storeVariable(identifier: "var3", value: ExpressionValue.of(10)!)
        let result: Result<MMExpression, ExpressionError> = MMExpression.build(
            expression: expressionSource,
            configuration: config
        )
    ...
    func validateSuccess() async throws {
        let expression: String = "CBRT(27.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double(3.0))
    }
