...
@Suite("Function SQRT()")
class FunctionSQRTTests {

    @Test("Definition of SQRT()")
    func validateInit() async throws {
        let f: FunctionSQRT = FunctionSQRT()
        #expect(f.id == FunctionIdentifier("FunctionSQRT"))
        #expect(f.symbols == ["SQRT"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "SQRT", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "SQRT(81.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double(9.0))
    }
