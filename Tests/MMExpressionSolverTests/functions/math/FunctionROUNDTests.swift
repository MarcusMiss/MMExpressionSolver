...
@Suite("Function ROUND()")
class FunctionROUNDTests {

    @Test("Definition of ROUND()")
    func validateInit() async throws {
        let f: FunctionROUND = FunctionROUND()
        #expect(f.id == FunctionIdentifier("FunctionROUND"))
        #expect(f.symbols == ["ROUND"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "ROUND", parameters:[
            ExpressionFunctionParameter(name: "value", strictTypes: [.double, .float, .decimal])
        ]))
    ...
    func evaluateInExpression(expressionSource: String) async throws {
        ...
        context.variables.storeVariable(identifier: "var2", value: ExpressionValue.of((Double(-10.123))))
        context.variables.storeVariable(identifier: "var3", value: ExpressionValue.of((Float(-10.123))))
        context.variables.storeVariable(identifier: "var4", value: ExpressionValue.of((Decimal(-10.123))))
    ...
    func validateSuccess() async throws {
        let expression: String = "ROUND(10.6)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double(11.0))
    }
