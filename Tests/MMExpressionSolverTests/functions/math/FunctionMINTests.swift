...
@Suite("Function MIN()")
class FunctionMINTests {

    @Test("Definition of MIN()")
    func validateInit() async throws {
        let f: FunctionMIN = FunctionMIN()
        #expect(f.id == FunctionIdentifier("FunctionMIN"))
        #expect(f.symbols == ["MIN"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "MIN", parameters:[
    ...
    @Test("Function evaluation in expression-context",
        arguments: [
          ...
          "MIN(10, 20)",
          "MIN(var1, 20)",
          "MIN(var2, 1000, -100, -200)",
          "MIN(var3, 100)",
          "MIN(var4, var3, var2, var1)",
        ]
    )
    ...
    func validateSuccess() async throws {
        let expression: String = "MIN(10.0, 20.0, 30.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
