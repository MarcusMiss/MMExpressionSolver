...
@Suite("Function CEIL()")
class FunctionCEILTests {

    @Test("Definition of CEIL()")
    func validateInit() async throws {
        let f: FunctionCEIL = FunctionCEIL()
        #expect(f.id == FunctionIdentifier("FunctionCEIL"))
        #expect(f.symbols == ["CEIL"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "CEIL", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "CEIL(10.6)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
