...
@Suite("Function FLOOR()")
class FunctionFLOORTests {

    @Test("Definition of FLOOR()")
    func validateInit() async throws {
        let f: FunctionFLOOR = FunctionFLOOR()
        #expect(f.id == FunctionIdentifier("FunctionFLOOR"))
        #expect(f.symbols == ["FLOOR"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "FLOOR", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "FLOOR(10.6)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double(10.0))
    }
