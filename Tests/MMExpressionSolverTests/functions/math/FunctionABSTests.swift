...
@Suite("Function ABS()")
class FunctionABSTests {

    @Test("Definition of ABS()")
    func validateInit() async throws {
        let f: FunctionABS = FunctionABS()
        #expect(f.id == FunctionIdentifier("FunctionABS"))
        #expect(f.symbols == ["ABS"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "ABS", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "ABS(-10.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
