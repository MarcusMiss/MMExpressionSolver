...
@Suite("Function LOG()")
class FunctionLOGTests {

    @Test("Definition of LOG()")
    func validateInit() async throws {
        let f: FunctionLOG = FunctionLOG()
        #expect(f.id == FunctionIdentifier("FunctionLOG"))
        #expect(f.symbols == ["LOG"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "LOG", parameters:[
