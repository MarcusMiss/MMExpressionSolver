...
@Suite("Function LOG1P()")
class FunctionLOG1PTests {

    @Test("Definition of LOG()")
    func validateInit() async throws {
        let f: FunctionLOG1P = FunctionLOG1P()
        #expect(f.id == FunctionIdentifier("FunctionLOG1P"))
        #expect(f.symbols == ["LOG1P"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "LOG1P", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "LOG1P(0.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
