...
@Suite("Function CFLT()")
class FunctionCFLTTests {

    @Test("Definition of CFLT()")
    func validateInit() async throws {
        let f: FunctionCFLT = FunctionCFLT()
        #expect(f.id == FunctionIdentifier("FunctionCFLT"))
        #expect(f.symbols == ["CFLT"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "CFLT", parameters:[
    ...
    func evaluateFunction() async throws {
        ...
        try #require(result.asFloat() == 100.0)
        // 02
        result = try f.evaluateFunction(expression: fake.get(),
                                        context: ctxt,
                                        functionToken: mockToken,
        ...
        try #require(result.isFloatValue == true)
        try #require(result.asFloat() == 100.123)
        // 03
        result = try f.evaluateFunction(expression: fake.get(),
                                        context: ctxt,
                                        functionToken: mockToken,
    ...
    func validateSuccess() async throws {
        let expression: String = "CFLT(100)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.isFloatValue)
        #expect(evalValue.asFloat()! == Float(100.0))
    }
