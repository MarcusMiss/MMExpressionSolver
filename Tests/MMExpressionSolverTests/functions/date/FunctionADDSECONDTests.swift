...
@Suite("Function ADDSECOND()")
class FunctionADDSECONDTests {

    @Test("Definition of ADDMONTH()")
    func validateInit() async throws {
        let f: FunctionADDSECOND = FunctionADDSECOND()
        #expect(f.id == FunctionIdentifier("FunctionADDSECOND"))
        #expect(f.symbols == ["ADDSECOND"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "ADDSECOND", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "ADDSECOND(DATE(2025, 9, 12, 11, 29, 0), 1)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        let context: ExpressionEvaluationContextImpl = ExpressionEvaluationContextImpl(configuration: config)
        let result: Result<MMExpression, ExpressionError> = MMExpression.build(
        ...
        #expect(evalValue.asDateTime()! == Date.create(year: 2025, month: 9, day: 12, hour: 11, minute: 29, second: 1))
    }
