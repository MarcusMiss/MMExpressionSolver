...
@Suite("Function GETHOUR()")
class FunctionGETHOURTests {

    @Test("Definition of GETHOUR()")
    func validateInit() async throws {
        let f: FunctionGETHOUR = FunctionGETHOUR()
        #expect(f.id == FunctionIdentifier("FunctionGETHOUR"))
        #expect(f.symbols == ["GETHOUR"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "GETHOUR", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "GETHOUR(DATE(2025, 9, 12, 11, 29, 0))"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        let context: ExpressionEvaluationContextImpl = ExpressionEvaluationContextImpl(configuration: config)
        let result: Result<MMExpression, ExpressionError> = MMExpression.build(
