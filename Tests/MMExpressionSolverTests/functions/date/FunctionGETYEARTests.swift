...
@Suite("Function GETYEAR()")
class FunctionGETYEARTests {

    @Test("Definition of ADDDAY()")
    func validateInit() async throws {
        let f: FunctionGETYEAR = FunctionGETYEAR()
        #expect(f.id == FunctionIdentifier("FunctionGETYEAR"))
        #expect(f.symbols == ["GETYEAR"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "GETYEAR", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "GETYEAR(DATE(2025, 9, 12, 11, 29, 0))"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        let context: ExpressionEvaluationContextImpl = ExpressionEvaluationContextImpl(configuration: config)
        let result: Result<MMExpression, ExpressionError> = MMExpression.build(
        ...
        #expect(evalValue.asInteger()! == 2025)
    }
