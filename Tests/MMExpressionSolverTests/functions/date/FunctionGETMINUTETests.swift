...
@Suite("Function GETMINUTE()")
class FunctionGETMINUTETests {

    @Test("Definition of GETMINUTE()")
    func validateInit() async throws {
        let f: FunctionGETMINUTE = FunctionGETMINUTE()
        #expect(f.id == FunctionIdentifier("FunctionGETMINUTE"))
        #expect(f.symbols == ["GETMINUTE"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "GETMINUTE", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "GETMINUTE(DATE(2025, 9, 12, 11, 29, 0))"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        let context: ExpressionEvaluationContextImpl = ExpressionEvaluationContextImpl(configuration: config)
        let result: Result<MMExpression, ExpressionError> = MMExpression.build(
