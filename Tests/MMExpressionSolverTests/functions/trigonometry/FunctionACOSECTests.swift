...
@Suite("Function ACOSEC()")
class FunctionACOSECTests {

    @Test("Definition of ACOSEC()")
    func validateInit() async throws {
        let f: FunctionACOSEC = FunctionACOSEC()
        #expect(f.id == FunctionIdentifier("FunctionACOSEC"))
        #expect(f.symbols == ["ACOSEC"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "ACOSEC", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "ACOSEC(1.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double(.pi / 2.0))
    }
