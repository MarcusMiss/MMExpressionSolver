...
@Suite("Function ASEC()")
class FunctionASECTests {

    @Test("Definition of TAN()")
    func validateInit() async throws {
        let f: FunctionASEC = FunctionASEC()
        #expect(f.id == FunctionIdentifier("FunctionASEC"))
        #expect(f.symbols == ["ASEC"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "ASEC", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "ASEC(1.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
