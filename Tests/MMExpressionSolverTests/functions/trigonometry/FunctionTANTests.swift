...
@Suite("Function TAN()")
class FunctionTANTests {

    @Test("Definition of TAN()")
    func validateInit() async throws {
        let f: FunctionTAN = FunctionTAN()
        #expect(f.id == FunctionIdentifier("FunctionTAN"))
        #expect(f.symbols == ["TAN"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "TAN", parameters:[
    ...
    func evaluateInExpressionWithFailure(expressionSource: String) async throws {
        ...
        print("Error: \(String(describing: result)) for '\(result)'")
    }
