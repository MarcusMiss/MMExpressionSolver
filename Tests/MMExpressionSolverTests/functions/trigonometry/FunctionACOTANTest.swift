...
@Suite("Function ACOTAN()")
class FunctionACOTANTest {

    @Test("Definition of TAN()")
    func validateInit() async throws {
        let f: FunctionACOTAN = FunctionACOTAN()
        #expect(f.id == FunctionIdentifier("FunctionACOTAN"))
        #expect(f.symbols == ["ACOTAN"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "ACOTAN", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "ACOTAN(0.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
