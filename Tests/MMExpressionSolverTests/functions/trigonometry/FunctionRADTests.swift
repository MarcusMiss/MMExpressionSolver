...
@Suite("Function RAD()")
class FunctionRADTests {

    @Test("Definition of RAD()")
    func validateInit() async throws {
        let f: FunctionRAD = FunctionRAD()
    ...
    func validateSuccess() async throws {
        let expression: String = "RAD(180.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double.pi)
    }
