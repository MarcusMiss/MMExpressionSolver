...
@Suite("Function SECH()")
class FunctionSECHTests {

    @Test("Definition of SECH()")
    func validateInit() async throws {
        let f: FunctionSECH = FunctionSECH()
        #expect(f.id == FunctionIdentifier("FunctionSECH"))
        #expect(f.symbols == ["SECH"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "SECH", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "SECH(0.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double(1.0))
    }
