...
@Suite("Function ATAN()")
class FunctionATANTests {

    @Test("Definition of TAN()")
    func validateInit() async throws {
        let f: FunctionATAN = FunctionATAN()
        #expect(f.id == FunctionIdentifier("FunctionATAN"))
        #expect(f.symbols == ["ATAN"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "ATAN", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "ATAN(0.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
