...
@Suite("Function ACOS()")
class FunctionACOSTests {

    @Test("Definition of ACOS()")
    func validateInit() async throws {
        let f: FunctionACOS = FunctionACOS()
        #expect(f.id == FunctionIdentifier("FunctionACOS"))
        #expect(f.symbols == ["ACOS"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "ACOS", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "ACOS(0.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
