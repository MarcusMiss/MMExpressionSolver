...
@Suite("Function ASINH()")
class FunctionASINHTests {

    @Test("Definition of ASINH()")
    func validateInit() async throws {
        let f: FunctionASINH = FunctionASINH()
        #expect(f.id == FunctionIdentifier("FunctionASINH"))
        #expect(f.symbols == ["ASINH"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "ASINH", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "ASINH(0.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
