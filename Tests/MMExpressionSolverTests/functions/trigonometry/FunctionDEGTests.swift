...
@Suite("Function DEG()")
class FunctionDEGTests {

    @Test("Definition of DEG()")
    func validateInit() async throws {
        let f: FunctionDEG = FunctionDEG()
    ...
    func validateSuccess() async throws {
        let expression: String = "DEG(PI/2.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double(90.0))
    }
