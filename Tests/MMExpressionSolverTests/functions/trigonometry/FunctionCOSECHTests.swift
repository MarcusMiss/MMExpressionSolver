...
@Suite("Function COSECH()")
class FunctionCOSECHTests {

    @Test("Definition of COSEC()")
    func validateInit() async throws {
        let f: FunctionCOSECH = FunctionCOSECH()
        #expect(f.id == FunctionIdentifier("FunctionCOSECH"))
        #expect(f.symbols == ["COSECH"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "COSECH", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "COSECH(1.0)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double(0.8509181282393216))
    }
