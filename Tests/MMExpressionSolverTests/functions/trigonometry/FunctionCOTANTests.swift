...
@Suite("Function COTAN()")
class FunctionCOTANTests {

    @Test("Definition of TAN()")
    func validateInit() async throws {
        let f: FunctionCOTAN = FunctionCOTAN()
        #expect(f.id == FunctionIdentifier("FunctionCOTAN"))
        #expect(f.symbols == ["COTAN"])
        #expect(f.definition == ExpressionFunctionDefinition(name: "COTAN", parameters:[
    ...
    func validateSuccess() async throws {
        let expression: String = "COTAN(0.017453292519943295)"
        let config: ExpressionConfiguration = ExpressionConfiguration.createDefault()
        ...
        #expect(evalValue.asDouble()! == Double(57.28996163075943))
    }
