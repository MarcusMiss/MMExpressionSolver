...
@Suite("InfixOperator || (OR)")
class InfixOperatorOrTests {

    @Test("Operator * definition")
    func operatorDefinition() async throws {
        let op: InfixOperatorOr = InfixOperatorOr()
        #expect(op.id == OperatorIdentifier("InfixOperatorOr"))
        #expect(op.symbols == ["||", "OR"])
