//
//  InfixOperatorEqualLessTests.swift
//  MMExpressionSolver
//
...
@Suite("InfixOperator <=")
class InfixOperatorEqualLessTests {
    
    @Test("Operator <= definition")
    func operatorDefinition() async throws {
        let op: InfixOperatorEqualLess = InfixOperatorEqualLess()
        #expect(op.id == OperatorIdentifier("InfixOperatorEqualLess"))
        #expect(op.symbols == ["<=", "=<", "≤"])
