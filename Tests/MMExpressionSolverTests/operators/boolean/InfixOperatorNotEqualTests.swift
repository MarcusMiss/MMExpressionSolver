//
//  InfixOperatorNotEqualTests.swift
//  MMExpressionSolver
//
...
@Suite("InfixOperator !=")
class InfixOperatorNotEqualTests {
    
    @Test("Operator != definition")
    func operatorDefinition() async throws {
        let op: InfixOperatorNotEqual = InfixOperatorNotEqual()
        #expect(op.id == OperatorIdentifier("InfixOperatorNotEqual"))
        #expect(op.symbols == ["<>", "!=", "≠"])
    ...
    func evaluateOperator() async throws {
        ...
        // 06: 100 != "Lorem" = false
        result = try op.evaluateOperator(context: ctxt, operatorToken: mockToken, arguments: [
            ExpressionValue.of(100), ExpressionValue.of("Lorem")
        ])
