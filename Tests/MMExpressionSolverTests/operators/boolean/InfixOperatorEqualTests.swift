...
@Suite("InfixOperator =")
class InfixOperatorEqualTests {

    @Test("Operator = definition")
    func operatorDefinition() async throws {
        let op: InfixOperatorEqual = InfixOperatorEqual()
        #expect(op.id == OperatorIdentifier("InfixOperatorEqual"))
        #expect(op.symbols == ["==", "="])
